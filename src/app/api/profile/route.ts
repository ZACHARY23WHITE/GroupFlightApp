...
export async function PATCH(req: Request) {
  // Username-only update — requires Firebase auth (guests can't have usernames)
  const uid = await verifyBearerUid(req);
  if (!uid) {
    return NextResponse.json({ error: "Sign in required to set a username." }, { status: 401 });
  }
  ...
  const normalized = normalizeUsername(b.username);
  const validation = validateUsername(normalized);
  if (!validation.ok) {
    return NextResponse.json({ error: validation.error }, { status: 400 });
  }

  const result = await claimUsername(uid, normalized);
  if (!result.ok) {
