...
export function ProfileForm() {
  ...
  const [, startUsernameTransition] = useTransition();
  ...
  useEffect(() => {
    const normalized = usernameDraft.trim().toLowerCase().replace(/[^a-z0-9_]/g, "");
    if (normalized.length < 3) {
      setUsernameAvail(normalized.length === 0 ? "idle" : "invalid");
      return;
    }
    ...
    const timer = setTimeout(() => {
      startUsernameTransition(() => {
        void fetch(`/api/users/search?q=${encodeURIComponent(normalized)}`)
          .then((r) => r.json())
          .then((j: { user: { uid: string } | null }) => {
            if (j.user && j.user.uid !== (user?.uid ?? "")) {
  ...
  }, [usernameDraft, user?.uid]);

  const saveUsername = useCallback(async () => {
    setUsernameError(null);
    setUsernameSuccess(false);
    const result = await setUsername(usernameDraft.trim());
    if (!result.ok) {
      setUsernameError(result.error);
      return;
    }
    setCurrentUsername(result.profile.username ?? null);
    setUsernameDraft("");
  ...
  return (
    ...
    <div className="relative mx-auto w-full max-w-lg px-4 pb-16 pt-8 sm:px-6">
      ...
      <div className="relative flex flex-col gap-8">
        ...
        {user ? (
          <div className="flex flex-col gap-4 rounded-2xl border border-stone-200/80 bg-white p-6 shadow-sm sm:p-7">
            <div>
              <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-stone-400">
                Username
              </p>
              <p className="mt-1 text-sm leading-relaxed text-stone-500">
                Friends can find you on Gathr by your username. Pick something unique.
              </p>
            </div>
            {currentUsername ? (
              <p className="text-sm text-stone-700">
                Your username:{" "}
                <span className="font-mono font-semibold text-stone-900">@{currentUsername}</span>
              </p>
            ) : null}
            <div className="flex flex-wrap items-center gap-2">
              ...
              <button
                type="button"
                disabled={usernameAvail !== "available"}
                onClick={() => void saveUsername()}
                className="rounded-xl bg-rose-600 px-4 py-3.5 text-sm font-semibold text-white shadow-sm transition-colors hover:bg-rose-700 disabled:opacity-45"
              >
            ...
            {usernameAvail === "checking" ? (
              ...
              <p className="text-xs font-semibold text-red-600">@{usernameDraft} is taken</p>
            ) : usernameAvail === "available" ? (
              <p className="text-xs font-semibold text-emerald-700">@{usernameDraft} is available</p>
            ) : usernameAvail === "invalid" ? (
              <p className="text-xs text-stone-400">At least 3 characters (letters, numbers, _)</p>
            ) : null}
            {usernameError ? (
              <p className="text-xs font-semibold text-red-600">{usernameError}</p>
            ) : null}
            {usernameSuccess ? (
              <p className="text-xs font-semibold text-emerald-700">Username saved!</p>
            ) : null}
          </div>
        ) : null}
