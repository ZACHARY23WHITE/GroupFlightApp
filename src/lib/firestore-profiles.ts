...
const usernamesCollection = () => getFirestoreDb().collection("usernames");
...
export function validateUsername(u: string): { ok: true } | { ok: false; error: string } {
  ...
  if (u.length > 20) return { ok: false, error: "Username must be 20 characters or less." };
  if (!/^[a-z0-9_]+$/.test(u)) return { ok: false, error: "Only letters, numbers, and underscores." };
  if (!/^[a-z0-9]/.test(u)) return { ok: false, error: "Username must start with a letter or number." };
  return { ok: true };
}

/** Atomically claims a username for a uid. Releases the old username if any. */
export async function claimUsername(
  ...
  try {
    await db.runTransaction(async (tx) => {
      const [profileSnap, newUsernameSnap] = await Promise.all([
        tx.get(profileRef),
        tx.get(newUsernameRef),
      ]);

      // If username is already taken by someone else, abort
      ...
      // Release old username if it's different
      if (oldUsername && oldUsername !== username) {
        tx.delete(db.collection("usernames").doc(oldUsername));
      }

      // Claim new username
      tx.set(newUsernameRef, { uid, displayName: profileSnap.data()?.displayName ?? "" });
...
/** Find a user by exact username. Returns null if not found. */
export async function findUserByUsername(username: string): Promise<UserSearchResult | null> {
  const snap = await usernamesCollection().doc(username).get();
