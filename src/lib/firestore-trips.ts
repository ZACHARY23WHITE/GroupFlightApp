...
export async function getTripWithTravelers(
  ...
  const pollDeadline = typeof data.pollDeadline === "string" ? data.pollDeadline : null;
  const phase: "voting" | "flights" =
    pollDeadline && new Date(pollDeadline) < new Date() ? "flights" : "voting";
  return {
    id: doc.id,
    name: String(data.name ?? ""),
    shareCode: String(data.shareCode ?? ""),
...
export async function updatePollDeadline(
  ...
  if (!doc.exists) return null;
  await ref.update({ pollDeadline: deadline ?? FieldValue.delete() });
