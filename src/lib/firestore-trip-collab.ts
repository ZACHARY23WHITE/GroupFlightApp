...
export type WinnersDto = {
  destination: ProposalDto | null;
  dates: ProposalDto | null;
};

export async function getWinners(tripId: string): Promise<WinnersDto> {
  const snap = await tripsCollection()
    .doc(tripId)
    .collection("proposals")
  ...
  const withVotes: { doc: FirebaseFirestore.QueryDocumentSnapshot; voteCount: number }[] = [];
  for (const d of snap.docs) {
    const votesSnap = await d.ref.collection("votes").get();
    withVotes.push({ doc: d, voteCount: votesSnap.size });
  }

  const byKind = (kind: "destination" | "dates") =>
    withVotes
      .filter((w) => w.doc.data().kind === kind)
      .sort((a, b) => b.voteCount - a.voteCount)[0] ?? null;

  const toDto = (w: { doc: FirebaseFirestore.QueryDocumentSnapshot; voteCount: number } | null): ProposalDto | null => {
    if (!w) return null;
    const x = w.doc.data();
    return {
      id: w.doc.id,
  ...
  return {
    destination: toDto(byKind("destination")),
    dates: toDto(byKind("dates")),
  };
}
