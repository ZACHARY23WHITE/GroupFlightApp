...
export async function setUsername(
  ...
  const auth = getFirebaseAuth();
  const u = auth?.currentUser;
  if (!u) return { ok: false, error: "Sign in required to set a username." };
  let token: string;
  try {
  ...
  if (!res.ok) return { ok: false, error: j.error ?? "Could not set username." };
  if (!j.profile) return { ok: false, error: "Could not set username." };
  return { ok: true, profile: j.profile };
}
